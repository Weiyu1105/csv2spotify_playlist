# -*- coding: utf-8 -*-
# Multi-playlist importer for Spotify (Python 3)
#
# Usage examples:
#   set SPOTIFY_OAUTH=Bearer YOUR_OAUTH_TOKEN
#   set SPOTIFY_USER_ID=your_spotify_user_id
...
#   --playlist-prefix   Prefix for playlist names (e.g., Lang | )
#   --playlist-suffix   Suffix for playlist names (e.g.,  (2025))
#   --token             OAuth token with 'Bearer ' prefix (or env SPOTIFY_OAUTH)
#   --user              Spotify user id (or env SPOTIFY_USER_ID)
#   --description       Description string for created playlists
#
# Notes:
# - File uses only ASCII quotes to avoid smart-quote issues (U+2018/U+2019).
# - Works on Windows (CRLF line endings), macOS, Linux.
...
def norm(s: str) -> str:
    ...
    # replace special quotes/dashes with spaces
    for ch in ["\"", "'", "“", "”", "‘", "’", "–", "—", "‐", "‑", "‒"]:
        s = s.replace(ch, " ")
    import re
    s = re.sub(r"[^\w\s]", " ", s)
...
def search_track(session, token: str, title: str, artists: List[str], album: str) -> Optional[str]:
    headers = {"Authorization": token}
    queries = []
    main_artist = artists[0] if artists else ""
    if album:
        queries.append('track:"%s" artist:"%s" album:"%s"' % (title, main_artist, album))
    if main_artist:
        queries.append('track:"%s" artist:"%s"' % (title, main_artist))
    queries.append('track:"%s"' % (title,))
...
def add_tracks(session, token: str, playlist_id: str, uris: List[str], dry_run: bool=False) -> None:
    ...
    for i in range(0, len(uris), 100):
        ...
        while True:
            ...
            if handle_rate(resp):
                continue
            if not resp.ok:
                print("[ERROR] Add chunk failed %s: %s" % (resp.status_code, resp.text), file=sys.stderr)
            break
...
def ensure_user_id(session, token: str, user_id: Optional[str]) -> str:
    if user_id:
        return user_id
    headers = {"Authorization": token}
...
def find_existing_playlist(session, token: str, name: str) -> Optional[str]:
    headers = {"Authorization": token}
...
def create_playlist(session, token: str, user_id: str, name: str, public: bool, description: str, dry_run: bool) -> str:
    if dry_run:
        print("[DRY] Would create/reuse playlist: %s" % name)
        return "DRY_PLAYLIST_ID"
    exist = find_existing_playlist(session, token, name)
    if exist:
        print("[OK ] Reusing existing playlist: %s (%s)" % (name, exist))
        return exist
    headers = {"Authorization": token, "Content-Type": "application/json"}
    url = CREATE_PLAYLIST_ENDPOINT.format(user_id=user_id)
    body = {"name": name, "public": public, "description": (description or "")[:300]}
    resp = session.post(url, headers=headers, data=json.dumps(body))
    if handle_rate(resp):
        resp = session.post(url, headers=headers, data=json.dumps(body))
    resp.raise_for_status()
    pid = resp.json().get("id")
    print("[OK ] Created playlist: %s (%s)" % (name, pid))
    return pid
...
def process_csv(session, token: str, playlist_id: str, csv_path: str, dry_run: bool=False) -> int:
    ...
    for row in rows:
        ...
        if uri:
            uris.append(uri)
            added += 1
            print("[FOUND] %s — %s" % (title, artists_field))
        else:
            print("[MISS ] %s — %s" % (title, artists_field))
    add_tracks(session, token, playlist_id, uris, dry_run=dry_run)
    print("[DONE] %s: matched %d/%d" % (csv_path, added, len(rows)))
    return added
...
def main():
    ...
    ap.add_argument("--public", action="store_true", help="Create as public")
    ...
    public = True if args.public and not args.private else False
    created_total = 0
    ...
    print("All done. Processed %d CSV files; created/reused %d playlists." % (len(args.csv), created_total))
